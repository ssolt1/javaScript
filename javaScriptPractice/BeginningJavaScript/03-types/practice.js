...
// `/`.
var celsToFahr = function (celTemp) {
  return ((celTemp * 9)/5 ) + 32;
};

var fahrToCels = function (fahTemp) {
...
var randBetween = function (max, min) {
...
var isSuit = function (suit) {
  var lowerCaseSuit = suit.toLowerCase();
...
var isRank = function (rank) {
  var lowerCaseRank = rank.toLowerCase();
...
var isCapitalized = function (str) {
  // my solution, compared actual charAt(0), to an upperCased version of charAt(0);
  //return str.charAt(0) === str.charAt(0).toUpperCase();
  // instructor created a range from cap 'A' to cap 'Z'
...
var isHTMLElement = function (elem) {
  // my solution is below, I used slice() function instead of substring().
  var openingTag = elem.slice(elem.indexOf("<") + 1, elem.indexOf(">"));
  var closingTag = elem.slice(elem.lastIndexOf("</") + 2, elem.lastIndexOf(">"));
  return openingTag === closingTag && elem.charAt(0) === "<" && elem.charAt(elem.length -1) === ">";
...
/* googled 'javascript difference between slice and substring and substr' sounds
like they mostly do the same thing, just a little differently.
the comments are slice handles negitives and overflows more gracefully than substring.
but substring perfoms better with large values.*/
